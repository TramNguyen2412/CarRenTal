...
////            "Tất cả", "Chờ xác nhận", "Đã xác nhận", "Đang thuê", "Đã trả xe", "Đã hủy"
////        });
...
////                    // Kiểm tra dữ liệu hợp đồng trước khi thêm vào bảng
////                    String tenKH = (hd.getTenKH() != null && !hd.getTenKH().isEmpty()) ? 
////                                  hd.getTenKH() : "Không xác định";
////
////                    String ngayLapStr = (hd.getNgayLap() != null) ? 
////                                       dateFormat.format(hd.getNgayLap()) : "";
////
////                    String trangThai = (hd.getTrangThai() != null) ? 
////                                      hd.getTrangThai() : "Không xác định";
////
...
////        
////        // Lấy danh sách hợp đồng
////        List<HopDong> danhSachHD = hopDongController.searchHopDong(keyword, trangThai);
////        
////        // Thêm dữ liệu vào bảng
////        for (HopDong hd : danhSachHD) {
////            modelHopDong.addRow(new Object[]{
...
////                if ("Đã trả xe".equals(hopDong.getTrangThai()) || "Đã hủy".equals(hopDong.getTrangThai())) {
////                    JOptionPane.showMessageDialog(this,
////                            "Không thể sửa hợp đồng đã trả xe hoặc đã hủy!",
////                            "Thông báo",
...
////            
////            if (hopDong != null) {
////                // Hiển thị chi tiết (chỉ xem, không sửa)
////                HopDongDialog dialog = new HopDongDialog(SwingUtilities.getWindowAncestor(this), hopDong, this);
...
//        tblHopDong.setFont(new Font("Arial", Font.PLAIN, 16));
...
//                    ((JLabel) comp).setFont(new Font("Arial", Font.PLAIN, 16));
//                }
//
...
//            System.out.println("Kết quả: " + danhSachHD.size() + " hợp đồng");
//            
//            // Thêm dữ liệu vào bảng
//            for (HopDong hd : danhSachHD) {
//                modelHopDong.addRow(new Object[]{
...
public class HopDongPanel extends JPanel {
    ...
    private void initComponents() {
        ...
        tblHopDong.setShowGrid(true);
        tblHopDong.setGridColor(new Color(230, 230, 230));
        tblHopDong.setFont(new Font("Arial", Font.PLAIN, 16)); // Tăng font size
        
        // Tùy chỉnh header bảng
        JTableHeader header = tblHopDong.getTableHeader();
        header.setFont(new Font("Arial", Font.BOLD, 16)); // Tăng font size
        header.setBackground(new Color(240, 240, 240));
        header.setForeground(new Color(60, 60, 60));
        header.setPreferredSize(new Dimension(0, 40));
        header.setBorder(BorderFactory.createMatteBorder(0, 0, 1, 0, new Color(220, 220, 220)));
        ...
        tblHopDong.setDefaultRenderer(Object.class, new DefaultTableCellRenderer() {
            ...
            public Component getTableCellRendererComponent(JTable table, Object value,
                ...
                if (comp instanceof JLabel) {
                    ((JLabel) comp).setBorder(BorderFactory.createEmptyBorder(0, 10, 0, 10));
                    ((JLabel) comp).setFont(new Font("Arial", Font.PLAIN, 16)); // Đảm bảo font size
                }
        ...
        try {
            ...
            if (selectedRow >= 0) {
                ...
                if (hopDong != null) {
                    // Hiển thị chi tiết (chỉ xem, không sửa)
                    HopDongDialog dialog = new HopDongDialog(SwingUtilities.getWindowAncestor(this), hopDong, this);
                    dialog.setTitle("Xem chi tiết hợp đồng: " + maHD);
                    dialog.setReadOnly(true);
                    dialog.setVisible(true);
                } else {
                    JOptionPane.showMessageDialog(this,
