...
public class CapNhatBaoDuongDialog extends JDialog {
    ...
    public CapNhatBaoDuongDialog(Window owner, PhieuBaoDuong phieu, BaoDuongPanel parentPanel) {
        super(owner, "Cập Nhật Phiếu Bảo Dưỡng", ModalityType.APPLICATION_MODAL);
        this.phieu = phieu;
    ...
    private void loadNhanVienToComboBox() {
        ...
        private void loadPhieuBaoDuong() {
            ...
            cboLoaiBD.setSelectedItem(phieu.getLoaiBD());
            dateNgayBD.setDate(phieu.getNgayBD());
            // Luôn clear và tạo mới list chi tiết để tránh cộng dồn
            chiTietList.clear();
            List<ChiTietBaoDuong> chiTietMoi = baoDuongController.getChiTietByMaBD(phieu.getMaBD());
            for (ChiTietBaoDuong ct : chiTietMoi) {
                chiTietList.add(new ChiTietBaoDuong(ct)); // clone để tránh trỏ cùng tham chiếu
            }
            chiTietTableModel.setData(chiTietList);
            updateTongTien();
        }
        ...
        private void showEditChiTietDialog(ChiTietBaoDuong chiTiet) {
            ...
            if (dialog.isSaved()) {
                ChiTietBaoDuong newCT = dialog.getChiTietBaoDuong();
                if (chiTiet == null) {
                    chiTietList.add(new ChiTietBaoDuong(newCT)); // clone để tránh lặp tham chiếu
                } else {
                    int idx = chiTietList.indexOf(chiTiet);
                    if (idx >= 0) chiTietList.set(idx, new ChiTietBaoDuong(newCT));
                }
                chiTietTableModel.setData(chiTietList);
                updateTongTien();
            }
    ...
    private void updateTongTien() {
        ...
        private void savePhieuBaoDuong() {
            ...
            // So sánh chi tiết hiện tại với chi tiết gốc trong DB
            List<ChiTietBaoDuong> chiTietGoc = baoDuongController.getChiTietByMaBD(maBD);
            boolean chiTietThayDoi = false;
            if (chiTietList.size() != chiTietGoc.size()) {
                chiTietThayDoi = true;
            } else {
                for (int i = 0; i < chiTietList.size(); i++) {
                    ChiTietBaoDuong ct1 = chiTietList.get(i);
                    ChiTietBaoDuong ct2 = chiTietGoc.get(i);
                    if (!ct1.getMaDV().equals(ct2.getMaDV()) || ct1.getSoLuong() != ct2.getSoLuong()) {
                        chiTietThayDoi = true;
                        break;
                    }
                }
            ...
            String result = null;
            String maPhieuMoi = null;
            if (phieu != null) {
                if (!chiTietThayDoi) {
                    // Chỉ cập nhật thông tin chung, không cập nhật lại chi tiết
                    result = baoDuongController.updatePhieuBaoDuongThongTinChung(
                        maBD, maXe, maKH, ngayBD, maNV, loaiBD
                    );
                } else {
            ...
            if (success) {
                PhieuBaoDuong phieuMoi = null;
                if (maPhieuMoi != null) {
                    phieuMoi = baoDuongController.getPhieuBaoDuongById(maPhieuMoi);
                }
                if (phieuMoi != null) {
                    lblTongTien.setText(String.format("%,.0f VNĐ", phieuMoi.getTongTienBD()));
                }
                JOptionPane.showMessageDialog(this, "Cập nhật phiếu bảo dưỡng thành công!\nTổng tiền: " + (phieuMoi != null ? String.format("%,.0f VNĐ", phieuMoi.getTongTienBD()) : ""));
                ...
                JOptionPane.showMessageDialog(this, "Cập nhật phiếu bảo dưỡng thất bại!", "Lỗi", JOptionPane.ERROR_MESSAGE);
            }
        }
}
